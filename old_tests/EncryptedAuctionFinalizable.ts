...
import { EncryptedAuctionFinalizable, EncryptedAuctionFinalizable__factory } from "../types";
...
describe("EncryptedAuctionFinalizable", function () {
  let signers: Signers;
  let contract: EncryptedAuctionFinalizable;
  ...
  beforeEach(async function () {
    ...
    const factory = (await ethers.getContractFactory(
      "EncryptedAuctionFinalizable",
    )) as EncryptedAuctionFinalizable__factory;

    contract = (await factory.deploy(auctionDuration, itemName)) as EncryptedAuctionFinalizable;
  ...
  it("finalizes auction and keeps Bob as winner", async function () {
    await submitEncryptedBid(signers.alice, 42);
